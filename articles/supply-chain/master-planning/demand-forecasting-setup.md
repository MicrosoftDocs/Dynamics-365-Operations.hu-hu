...
Az igény-előrejelzés több vállalatot is képes generálni. A vállalatok, amelyek együtt vannak tervezve, ugyanannak a vállalatközi Dynamics 365 Supply Chain Management tervezőcsoportnak abe vannak csoportosítva. Annak megadásához, hogy vállalatonként mely cikkfelosztási kulcsokat kell figyelembe venni az igény-előrejelzésben, társítsa a cikkfelosztási kulcsot a vállalatközi tervezési csoport taghoz.
...
Az igény-előrejelzési paraméterek beállításához kattintson az **Alaptervezés beállítása igény-előrejelzési \>\>\>** paramétereire. Mivel az igény-előrejelzés működése vállalatközi, ezért a telepítse globális. Más szóval az összes jogi személyre (vállalatra) vonatkozik.
...
Az igény-előrejelzési modellek alapértelmezett paramétereinek és értékeinek beállításához használja az Alaptervezés beállítása igény-előrejelzési **\>\>\> algoritmusának** paramétereit. A rendszer szabványos paraméterkészletet biztosít. Mindegyik paraméter a következő mezőket tartalmazza:
...
- **Hiányzó érték-helyettesítés** – Meghatározza a múltbeli adatok hiányosságainak kitöltésének módját. Lehetőségek: *(numerikus érték)*, *ÁTLAGOS*, *ELŐZŐ*, FRANCIA *LINEÁRIS*, *FRANCIAATE POLINOMIÁLIS.*
...
- A munkaterületnek a Microsoft által biztosított parancsfájl futtatásával való beállításához hajtsa végre az 1. beállításnál megadott utasításokat: Parancsfájl futtatása a gépi tanulás munkaterületének automatikus beállításához, majd előreugrás az Azure-gépi tanulás szolgáltatás kapcsolati paramétereinek beállításához az Ellátásilánc-kezelés [...](#ml-workspace-script)[szakaszban](#demand-forecast-parameters).
- A munkaterület manuális beállításához kövesse a 2. lehetőségnél megadott utasításokat: állítsa be manuálisan a gépi tanulás munkaterületének szakaszát, majd ugorjon előre az Azure-számítógép-oktató szolgáltatás kapcsolati paramétereinek beállítása az Ellátásilánc-kezelés [...](#ml-workspace-manual)[szakaszban](#demand-forecast-parameters). Ez a beállítás több időt vesz igénybe, de több ellenőrzést ad.
...
Ez a szakasz bemutatja, hogyan lehet beállítani a gépi oktatási munkaterületet a Microsoft által biztosított automatikus parancsfájl használatával. Ha azt szeretné, hogy manuálisan állítsa be az összes erőforrást, kövesse a 2. lehetőségben található utasításokat: Állítsa be kézzel a gépi tanulás [munkaterületének](#ml-workspace-manual) szakaszát. Nem kell mindkét lehetőséget végrehajtania.

1. Nyissa meg a Sablonokat igény-előrejelzéshez az [Dynamics 365 Supply Chain Management Azure-számítógép oktatási tárházában (repo), és töltse le a](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service) következő fájlokat:
...
1. A munkaterület már be van állítva. Ugorjon előre az [Azure-számítógép-oktatási szolgáltatás kapcsolati paramétereinek beállításához az Ellátásilánc-kezelés](#demand-forecast-parameters) szakaszban.
...
További tájékoztatás: [Munkaterület](/azure/machine-learning/quickstart-create-resources#create-the-workspace) létrehozása.
...
1. Nyissa meg a Sablonokat igény-előrejelzéshez az Azure-gépi oktatófájllal, és töltse le [Dynamics 365 Supply Chain Management a](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service)**sampleInput.csv** fájlt.
1. Nyissa meg az 1. lépésben létrehozott tárolási [fiókot: hozzon létre egy új](#create-workspace) munkaterületet.
1. A [...](/azure/storage/blobs/storage-quickstart-blobs-portal#create-a-container)*Demplan-azureml nevű tároló létrehozásához kövesse a Tároló létrehozása* útmutatását.
1. Töltse fel az 1. lépésben letöltött sampleInput.csv fájlt a most létrehozott **tárolóba**. Ez a fájl szükséges a csővezeték közzétételéhez és a teszt-előrejelzés generálásához. Az utasításokat [lásd: Blokk blob](/azure/storage/blobs/storage-quickstart-blobs-portal#upload-a-block-blob) feltöltése.
...
1. Nyissa meg az 1. lépésben létrehozott gépi oktatási munkaterület részletező [lapját: Hozzon létre egy új](#create-workspace) munkaterületet. Keresse meg **a Studio web** URL-címét, és válassza ki a megnyitni kívánt hivatkozást.
...
1. Nyissa meg a Sablonokat igény-előrejelzéshez az Azure Machine Learning repo segítségével, és töltse le [Dynamics 365 Supply Chain Management a következő](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service) fájlokat:
...
1. Az Azure szolgáltatásban nyissa meg és tekintse parameters.py az éppen feltöltött **fájlban**. Győződjön meg róla, hogy az érték 1-ével kisebb, mint a 4. lépésben a számítási fürthöz konfigurált érték: A Számított erőforrások `nodes_count`[szakasz](#config-compute-resources) konfigurálása. Ha az érték nagyobb vagy egyenlő a számítási fürtben található csomópontok számán, akkor a csővezeték-futás el is `nodes_count` indítható. Ettől azonban le fog állni, amíg a szükséges erőforrásokra várakozik. A csomópontok számról a [4. lépés: Számítási erőforrások konfigurálása szakasz tartalmaz](#config-compute-resources) további tudnivalókat.
1. Válassza ki **api_trigger most feltöltött api_trigger.fájlját,** majd futtassa. Létrehoz egy csővezetéket, amely az API-n keresztül indítható el.
...
1. Jelentkezzen be az Azure-portálra.
1. Új pályázat regisztrálása a bérlő Azure Active Directory Azure AD () számára. Az útmutatásért lásd a portálon az erőforrásokhoz hozzáférő alkalmazás és szolgáltatásnév [Azure AD](/azure/active-directory/develop/howto-create-service-principal-portal) létrehozásához.
1. A varázsló befejezése után kövesse a képernyőn látható utasításokat. Az alapértelmezett beállítások használata.
1. Adjon új Active Directory-alkalmazáselérést a következő erőforrásokhoz, amelyek az Azure rendszer gépi oktatásának beállítása szakaszban [létre vannak](#ml-workspace) hozva. Az útmutatás az [Azure-szerepkörök hozzárendelése az Azure portálon](/azure/role-based-access-control/role-assignments-portal?tabs=current) található. Ez a lépés lehetővé teszi, hogy a rendszer importálja és exportálja az előrejelzési adatokat, és elindítsa az ellátásilánc-kezelésből a gépi oktatási folyamatot.
...
1. A létrehozott alkalmazás Tanúsítványok & szakaszának létrehozásához hozzon létre egy **titkos** alkalmazást. Az útmutatásért lásd az Ügyfél titkos [hozzáadása gombra vonatkozó](/azure/active-directory/develop/quickstart-register-app#add-a-client-secret) tudnivalókat.
...
1. Az **Azure-számítógép oktatási szolgáltatás lapján állítsa** be a következő mezőket:
    ...
    - **Bérlő azonosítója** – adja meg az Azure-bérlő azonosítóját. Az ellátásilánc-kezelés ezt az azonosítót fogja használni az Azure-számítógép-oktató szolgáltatással való hitelesítésre. A bérlőazonosító az **Azure-portál Áttekintés** lapján Azure AD található.
    - **Szolgáltatásnév alkalmazásazonosítója – adja meg annak az alkalmazásnak az azonosítóját, amit az** Active Directory alkalmazás szakaszban [létrehozott](#aad-app). Ez az érték az Azure számítógép-oktató szolgáltatásnak való API-kérések engedélyeztetését teszi lehetővé.
    ...
    - **Tárolási fiók neve – adja meg azt az Azure-tárolási fióknevet, amely a beállítási varázsló Azure-munkaterületen való** futtatásakor meg van adva. (További tájékoztatás: [Gépi tanulás beállítása az Azure](#ml-workspace) szakaszban.)
    - **Térér végpontjának címe – adja meg az Azure-számítógép oktatási szolgáltatása térmében** található REST végpont URL-címét. Ez a csővezeték az Utolsó lépés, amikor gépi tanulást hoz létre [az Azure](#ml-workspace) szolgáltatásban. A csővezeték URL-címének bejéhez jelentkezzen be az Azure-portálra, és válassza ki a **navigációs** csővezeték-t. A T csővezeték lapon válassza ki **a** **TriggerDemandForecastGeneration nevű csővezeték-végpontot.** Ezután másolja át a megjelenő REST-végpontot.
