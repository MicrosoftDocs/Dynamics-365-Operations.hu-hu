...
| A következő konszolidációs mező határozza meg egy *átmozgatási sorhoz* tartozó szállítmányok konszolidálásához használt értékek egyedi kombinációját. (Az összes többi mezőt figyelmen kívül hagyja a program.)<ul><li>Megrendelés száma (OrderNum)</li></ul> | A következő konszolidációs mezők határozzák meg egy *átmozgatási sorhoz* tartozó szállítmányok konszolidálásához használt értékek egyedi kombinációját. (Az összes többi mezőt figyelmen kívül hagyja a program.)<ul><li>Megrendelés száma (OrderNum)</li><li>Szállítás címzettje (DeliveryName)</li><li>Postai cím (DeliveryPostalAddress)</li><li>ISO ország kódja (CountryRegionISOCode)</li><li>Cím (Address)</li><li>Telephely (InventSiteId)</li><li>Raktár (InventLocationId)</li><li>Szállítmányozó (CarrierCode)</li><li>Szállítmányozói szolgáltatás (CarrierServiceCode)</li><li>Szállítás módja (ModeCode) \*</li><li>Szállítmányozói csoport (CarrierGroupCode)</li><li>Szállítási feltételek (DlvTermId)</li></ul><p>Csak ezek a mezők az új szállítmány létrehozásakor rendelkezésre álló és inicializált mezők.</p><p>\* Megjegyzés: *A ModeCode* az **·** **átviteli** sorhoz kiválasztott szállítmányozóhoz rendelt mód (**nem** pedig az átszállítási sorhoz kiválasztott szállítási mód). *Ha azt választja,* **hogy a konszolidációs feltételek között szerepeljen a Szállítás módja (ModeCode** **·** **·**), akkor a rendszer csak az azonos módértékkel megadott szállítási módú sorokat vonja össze a szállítmányozóval, **a** szállítmányozó szolgáltatással és a szállítási móddal együtt meghatározottak (az értékektől függetlenül). Ezenkívül a rendszer összesíteni **fogja** az összes olyan átviteli sort is, ahol a mód üres.</p> |
| A következő konszolidációs mezők határozzák meg egy *értékesítési sorhoz* tartozó szállítmányok konszolidálásához használt értékek egyedi kombinációját. (Az összes többi mezőt figyelmen kívül hagyja a program.)<ul><li>Megrendelés száma (OrderNum)</li><li>Vevő hivatkozása (CustomerRef)</li><li>Vevői igénylés (CustomerReq)</li><li>Szállítási feltételek (DlvTermId)</li></ul> | A következő konszolidációs mezők határozzák meg egy *értékesítési sorhoz* tartozó szállítmányok konszolidálásához használt értékek egyedi kombinációját. (Az összes többi mezőt figyelmen kívül hagyja a program.)<ul><li>Megrendelés száma (OrderNum)</li><li>Számlaszám (AccountNum)</li><li>Szállítás címzettje (DeliveryName)</li><li>Postai cím (DeliveryPostalAddress)</li><li>ISO ország kódja (CountryRegionISOCode)</li><li>Cím (Address)</li><li>Telephely (InventSiteId)</li><li>Raktár (InventLocationId)</li><li>Szállítmányozó (CarrierCode)</li><li>Szállítmányozói szolgáltatás (CarrierServiceCode)</li><li>Szállítás módja (ModeCode) \*</li><li>Szállítmányozói csoport (CarrierGroupCode)</li><li>Ügynök azonosítója (BrokerCode)</li><li>Irány (LoadDirection)</li><li>Szállítási feltételek (DlvTermId)</li><li>Vevő hivatkozása (CustomerRef)</li><li>Vevői igénylés (CustomerReq)</li></ul><p>Csak ezek a mezők az új szállítmány létrehozásakor rendelkezésre álló és inicializált mezők.</p><p>\* Megjegyzés: *A ModeCode* az **·** **értékesítési** sorhoz kiválasztott szállítmányozóhoz rendelt mód (**nem** pedig az értékesítési sorokhoz kiválasztott szállítási mód). *Ha azt választja,* **hogy a konszolidációs feltételek között szerepeljen a Szállítás módja (ModeCode** **·** **·**), akkor a rendszer csak olyan értékesítési sorokat fog konszolidálni, amelyeknél a szállítmányozó, **a** Szállítmányozói szolgáltatás és a Szállítás módja meg van adva a sorhoz (értéküktől függetlenül). Ezenkívül a rendszer összesíteni **fogja az összes olyan értékesítési sort is, ahol** a mód üres.</p> |
