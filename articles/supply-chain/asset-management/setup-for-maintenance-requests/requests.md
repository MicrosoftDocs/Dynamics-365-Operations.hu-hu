---
title: Karbantartási kérések
description: Ez a témakör áttekintést ad az Eszközkezelés modul karbantartási kéréseiről.
author: josaw1
manager: tfehr
ms.date: 07/26/2019
ms.topic: article
ms.prod: ''
ms.service: dynamics-ax-applications
ms.technology: ''
ms.search.form: ''
audience: Application User
ms.reviewer: kamaybac
ms.custom: ''
ms.assetid: ''
ms.search.region: Global
ms.author: riluan
ms.search.validFrom: 2019-07-30
ms.dyn365.ops.version: AX 7.0.0
ms.openlocfilehash: 5e8a7efcc2d4033a6cb1306c665af5eb1b0ba522
ms.sourcegitcommit: deac22ba5377a912d93fe408c5ae875706378c2d
ms.translationtype: HT
ms.contentlocale: hu-HU
ms.lasthandoff: 01/16/2021
ms.locfileid: "5022005"
---
# <a name="maintenance-requests"></a>Karbantartási kérések

[!include [banner](../../includes/banner.md)]

 

Lehetősége van beállítani különböző típusú, a vállalatnál használható kéréseket. Az egyik ilyen típus a karbantartási kérés. A karbantartási kérések az adott feladathoz szükséges munkarendelés igénylésére szolgálnak. Karbantartási kérést minden olyan felhasználó létrehozhat, aki hozzáfér az Eszközkezelés funkcióhoz. Egy tervező vagy vezető ezután eldöntheti, hogy a karbantartási kérést át kell-e alakítani munkarendeléssé, vagy ki kell-e töltenie saját életciklusát.

## <a name="view-maintenance-requests"></a>Karbantartási kérések megtekintése

- Válassza az **Eszközkezelés** \> **Közös** \> **Karbantartási kérések** \> **Összes karbantartási kérés** vagy **Aktív karbantartási kérések** lehetőséget.

    Az **Összes karbantartási kérés** listaoldalon az összes karbantartási kérés látható, függetlenül azok életciklus-állapotától. Az **Aktív karbantartási kérések** listaoldalon csak az aktív állapotú karbantartási kérések jelennek meg.

A következő ábra az **Összes karbantartási kérés** listaoldalt szemlélteti.

![Összes karbantartási kérés listaoldal](media/01-setup-for-requests.png)


[!INCLUDE[footer-include](../../../includes/footer-banner.md)]