...
A Pénzügyi információk a Microsoft Dynamics 365 Finance és a Common Data Service, az Azure és az AI Builder funkcióit kombinálva hatékony előrejelző eszközöket biztosítanak a szervezet számára. Ez a témakör azokat a konfigurációs lépéseket ismerteti, amelyek lehetővé teszik a rendszer számára a Pénzügyi információkban elérhető képességek használatát.
...
## <a name="configure-common-data-service"></a>Common Data Service konfigurálása
...
1. Nyissa meg a [Power Platform felügyeleti központot](https://admin.powerplatform.microsoft.com/), és az alábbi lépések végrehajtásával hozzon létre egy új Common Data Service környezetet ugyanabban az Active Directory-bérlőben:
    ...
    11. Jegyezze fel a Common Data Service szervezetazonosítót.
    ...
    16. Állítsa be a **Példányhivatkozási adatok azonosítója** mezőt arra a Common Data Service szervezeti azonosítóértékre, amelyről korábban feljegyzést készített.
    17. A böngésző címsorában jegyezze fel a Common Data Service szervezet URL-címét. Az URL-cím lehet például `https://org42b2b3d3.crm.dynamics.com`.
...
### <a name="enter-the-common-data-service-directory-id-and-the-users-azure-ad-object-id"></a>Adja meg a Common Data Service címtárazonosítót és a felhasználó Azure AD objektumazonosítóját

1. Adja meg a Common Data Service címtárazonosítót:

    1. Nyissa meg az [Azure-portált](https://portal.azure.com).
    2. Jelentkezzen be a környezet létrehozásához használt felhasználói Common Data Service azonosítóval.
...
> [!NOTE]
> Győződjön meg arról, hogy a következő erőforrásokat hozza létre ugyanabban a Azure AD-példányban, mint a Common Data Service-környezetben. Más Azure AD-példányból származó erőforrások nem használhatók.
...
4. Az **Információk megszerzése** bővítmény részletei oldalon adja meg az alábbi értékeket.
    ...
    | Érték                                                    | Leírás |
    |----------------------------------------------------------|-------------|
    | CDS szervezet URL-címe                                     | A Common Data Service szervezet URL-címe a Common Data Service-példányból. Ha meg szeretné találni ezt az értéket, nyissa meg a [Power Apps-portált](https://make.powerapps.com), és válassza a jobb felső sarokban látható **Beállítások** gombot (fogaskerék szimbólum), válassza ki a **Speciális beállításokat**, majd másolja az URL-címet. (Az URL-cím a következőre végződik: „dynamics.com”). |
    | CDS szerv. azon.                                               | A Common Data Service-példány környezeti azonosítója. Ha meg szeretné találni ezt az értéket, nyissa meg a [Power Apps-portált](https://make.powerapps.com), és válassza a jobb felső sarokban látható **Beállítások** gombot (fogaskerék szimbólum), válassza ki a **Testreszabás \> Fejlesztői erőforrások \> Példány hivatkozási információja lehetőséget**, majd másolja az **azonosító** értéket. |
    | CDS-bérlő azonosítója (címtárazonosító az AAD-ből)               | A Common Data Service-példány bérlőazonosítója. Ha meg szeretné találni ezt az értéket, nyissa meg az [Azure-portál](https://portal.azure.com) lehetőséget, menjen ide: **Azure Active Directory**, és másolja át a **Bérlőazonosító** értéket. |
    | Adja meg a rendszergazdai szerepkörrel rendelkező felhasználó objektumazonosítóját | Az Azure AD-felhasználó felhasználói objektumazonosítója a Common Data Service-ben. Ennek a felhasználónak a Common Data Service-példány rendszergazdájának kell lennie. Ha meg szeretné találni ezt az értéket, nyissa meg az [Azure-portál](https://portal.azure.com) lehetőséget, menjen a **Azure Active Directory \> Felhasználók** lehetőségre, válassza ki a felhasználót, majd az **Identitás** szakaszba másolja be az **Objektumazonosító** értékét. |
    | Ez az alapértelmezett CDS-környezet a bérlő számára?      | Ha a Common Data Service-példány a létrehozott első termelési példány, jelölje be ezt a jelölőnégyzetet. Ha a Common Data Service-példányt manuálisan hozták létre, törölje a jelet a jelölőnégyzetből. |
