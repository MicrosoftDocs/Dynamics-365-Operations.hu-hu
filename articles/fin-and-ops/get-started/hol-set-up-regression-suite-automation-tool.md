...
- Annak megértése, hogy a Microsoft Dynamics 365 for Finance and Operations Feladatrögzítő funkciója a Microsoft Dynamics Lifecycle Services (LCS) szolgáltatásával és a Microsoft Azure DevOps szolgáltatással együtt lehetővé teszi a tesztesetek létrehozását, konfigurálását, futtatásáv, vizsgálatát és a róluk szóló jelentések készítését.
- Funkcionális felhasználók bátorítása arra, hogy az üzleti feladatokat a Feladatrögzítővel rögzítsék a Finance and Operations szolgáltatásban, majd a feladatrögzítéseket automatizált testek sorozatává alakítsák forráskód írása nélkül.

## <a name="before-you-begin"></a>Előzetes feladatok

### <a name="prerequisites"></a>Előfeltételek

- Az oktatóanyaghoz olyan Finance and Operations környezet szükséges, amely a Microsoft Dynamics 365 for Finance and Operations 10.0-s (2019. áprilisi) vagy újabb verzióját futtatja. A Microsoft Dynamics 365 for Finance and Operations, Enterprise edition 7.3 szolgáltatást használó ügyfelek számára a Platform update 20 (PU 20) vagy újabb verzió is támogatott.
- A felhasználónak rendszergazdai jogosultsággal kell rendelkeznie a Finance and Operations környezethez.
...
## <a name="finance-and-operations-environment"></a>Finance and Operations környezet

### <a name="version-requirement"></a>Verziókövetelmények

10-es verziót futtató Finance and Operations tesztkörnyezet szükséges. A 7.3-as verziót használó ügyfelek számára a Platform update 20 vagy újabb verzió is támogatott.

> [!NOTE]
> Az RSAT szolgáltatásnak hozzá kell férnie a Finance and Operations tesztkörnyezethez internetböngészőn keresztül.
...
Ezt a lépést szükséges megtenni annak érdekében, hogy csatlakozhasson az LCS-hez, így a feladatrögzítéseket az LCS megfelelő BPM-tárába tudja menteni a Finance and Operations-kliensen keresztül.

1. Nyissa meg a Finance and Operations-klienst.
...
> [!NOTE]
> Győződjön meg róla, hogy az összes feladatrögzítés a Finance and Operations fő irányítópultján kezdődik. Az egyes rögzítések legyenek rövidek és egy üzleti feladatra koncentráljanak, például egy értékesítési rendelés létrehozásához.
...
Az RSAT minden olyan számítógépre telepíthető, amelyen fut a Windows 10, és amely webböngészőn (Internet Explorer vagy Google Chrome) keresztül csatlakozhat a Finance and Operations környezethez.
...
> [!NOTE]
> Ha a Finance and Operations környezet egy 2. vagy magasabb szintű környezet, a tanúsítvány ujjlenyomatát frissíteni kell a wif.config fájlban a környezet **összes** AOS-számítógépéhez.
...
4. A **wif.config** fájl frissítéséhez adjon hozzá egy új hitelesítésszolgáltatói bejegyzést a tanúsítványhoz és a hitelesítésszolgáltató nevét, amint az a következő példában látható.
    ...
    > [!NOTE]
    > Ha több felhasználó ugyanazt a Finance and Operations alkalmazást használja, akkor mindegyik felhasználónak külön ujjlenyomatot kell létrehoznia, és ezeket a ujjlenyomatokat is hozzá kell adni a **\<kulcsok\>** szakaszban.
...
Ha egy 2. vagy magasabb szintű (szabványos elfogadási tesztkörnyezet vagy magasabb) Finance and Operations környezet van használatban, ellenőrizze vagy állítsa be a következő jegyzékbeállítást azon a számítógépen, amelyen a RSAT telepítve van. Ezt a lépést kötelező végrehajtani, mert segít elkerülni a hitelesítési vagy RSAT-kapcsolati hibákat.
...
3. Válassza a jobb felső sarokban a **Beállítások** gombot (fogaskerék szimbólum), majd a megjelenő párbeszédpanelen töltse ki a következő mezőket:
    ...
    - **Állomásnév** – adja meg a Finance and Operations tesztkörnyezet állomásnevét, például **\<myaos\>.cloudax.dynamics.com**. Ne szerepeljen a **https://** vagy **http://** előtag.
    - **SOAP állomásnév** – adja meg a Finance and Operations tesztkörnyezet SOAP-állomásnevét. A SOAP-állomásnév általában ugyanaz, mint az állomásnév, de van **soap** -utótagja. Itt egy példa: **\<myaos\>soap.cloudax.dynamics.com**. Ne szerepeljen a **https://** vagy **http://** előtag.
    ...
    - **Rendszergazda felhasználóneve** – Adja meg a rendszergazda felhasználó e-mail-címét a Finance and Operations tesztkörnyezetben.
    ...
    - **Vállalatnév** – Adja meg annak a vállalatnak a nevét, amelyet alapértelmezett Finance and Operations vállalatként kell használni az Excel-paraméterfájlok létrehozásakor. A vállalat később módosítható az Excel-paraméterfájl szerkesztésével.
...
3. Ha minden egyes futtatáshoz egyedi termékazonosítót szeretne létrehozni anélkül, hogy minden alkalommal újra meg kelljen nyitni az Excel-paraméterfájlt, és manuálisan kelljen frissíteni a termékszámot, használja a következő Excel-formulát.
    ...
    > [!NOTE]
    > Az **Általános** lap mellett az Excel-paraméterfájl is tartalmaz egy adatlapot minden Finance and Operations űrlapoldalhoz, amelyet a teszteset meglátogat.
