...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="b6265-103">Rendszerindítás vállalati adatokkal – GYIK</span><span class="sxs-lookup"><span data-stu-id="b6265-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [rename-banner](~/includes/cc-data-platform-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="b6265-104">Miért van szükség rendszerindításra?</span><span class="sxs-lookup"><span data-stu-id="b6265-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="b6265-105">Előfordulhat, hogy üzleti adatokkal rendelkező Dataverse alkalmazáspéldányt vagy más Dynamics 365-alkalmazáspéldányt használ, és engedélyezni szeretné a kettős írású kapcsolatot hozzá.</span><span class="sxs-lookup"><span data-stu-id="b6265-105">You might have an existing Dataverse or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="b6265-106">Ebben az esetben a kettős írású kapcsolat engedélyezése előtt rendszerindítást kell végezni a Dataverse- vagy a másik Dynamics 365-alkalmazásadatainál a vállalati információkkal.</span><span class="sxs-lookup"><span data-stu-id="b6265-106">In this case, you need to bootstrap Dataverse or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="b6265-107">Mikor kell használni a rendszerindítást?</span><span class="sxs-lookup"><span data-stu-id="b6265-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="b6265-108">A kettős írású táblaleképezések engedélyezése előtt kell rendszerindítást használni (az 5. lépés során).</span><span class="sxs-lookup"><span data-stu-id="b6265-108">You should use bootstrapping before enabling dual-write table maps (during step #5).</span></span>  
1. <span data-ttu-id="b6265-109">Ha a Finance and Operations alkalmazás és a Dataverse vagy másik Dynamics 365-alkalmazás példányai között be szeretné állítani a kettős írású kapcsolatot, jelentkezzen be a Finance and Operations alkalmazásba rendszergazdaként.</span><span class="sxs-lookup"><span data-stu-id="b6265-109">To setup the dual-write connection between instances of your Finance and Operations app and the Dataverse or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="b6265-110">Lépjen az **Adatkezelés** modulba, és kattintson a **Kettős írás** gombra.</span><span class="sxs-lookup"><span data-stu-id="b6265-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="b6265-111">Ez elindítja az **Adatintegrátort**.</span><span class="sxs-lookup"><span data-stu-id="b6265-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="b6265-112">Hozzon létre kettős írású kapcsolatot legalább egy vállalathoz.</span><span class="sxs-lookup"><span data-stu-id="b6265-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="b6265-113">![Kettős írású kapcsolat létrehozása](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="b6265-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="b6265-114">Engedélyezze a **Cdm_companies** táblaleképezést.</span><span class="sxs-lookup"><span data-stu-id="b6265-114">Enable the **Cdm_companies** table map.</span></span> <span data-ttu-id="b6265-115">Ez szinkronizálja a vállalatokat a Finance and Operations alkalmazásból a következőbe: Dataverse.</span><span class="sxs-lookup"><span data-stu-id="b6265-115">This synchronizes companies from the Finance and Operations app to Dataverse.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="b6265-116">![Táblaleképezés engedélyezése](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="b6265-116">![Enable the table map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="b6265-117">Futtassa a rendszerindítási mintakódot a Dataverse vagy a másik Dynamics 365-alkalmazás példányában.</span><span class="sxs-lookup"><span data-stu-id="b6265-117">Run the sample bootstrapping code on the Dataverse or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="b6265-118">Amikor a rendszerindítás után a rendszer készen áll az élő szinkronizálásra, engedélyezze az táblaleképezést.</span><span class="sxs-lookup"><span data-stu-id="b6265-118">When the bootstrapping is done and the system is ready for the live sync, enable the table maps.</span></span>  

    <span data-ttu-id="b6265-119">Az táblaleképezés engedélyezése elindítja a kezdeti adatszinkronizálási műveletet az engedélyezett táblaleképezésekhez.</span><span class="sxs-lookup"><span data-stu-id="b6265-119">Enabling the table maps triggers the initial data sync for the enabled table maps.</span></span> <span data-ttu-id="b6265-120">A program szinkronizálja a kettős írású kapcsolatnál kiválasztott vállalatokra vonatkozó adatokat a Finance and Operations és a Dataverse között.</span><span class="sxs-lookup"><span data-stu-id="b6265-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Dataverse.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="b6265-121">Hogyan használható a kódminta?</span><span class="sxs-lookup"><span data-stu-id="b6265-121">How to I use the code sample?</span></span>
<span data-ttu-id="b6265-122">A mintakód egy olyan C#-alkalmazás, amely betölthető a Visual Studio rendszerben.</span><span class="sxs-lookup"><span data-stu-id="b6265-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="b6265-123">NuGet-csomagfüggőségekre van szükség a Dataverse szoftverfejlesztői készlethez (SDK), ha szokásos Visual Studio-eszközökkel szeretne frissíteni.</span><span class="sxs-lookup"><span data-stu-id="b6265-123">It takes NuGet package dependencies on the Dataverse SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="b6265-124">Miután kitömörítette és megnyitotta a megoldást a Visual Studio rendszerben, illetve helyreállította a NuGet-csomagokat, keresse meg a **TENNIVALÓ** részt a kódban.</span><span class="sxs-lookup"><span data-stu-id="b6265-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="b6265-125">A vállalati adatok rendszerindítására vonatkozó összes döntés esetében **TENNIVALÓ** jegyzet tartozik, valamint a szabályos megvalósítást bemutató mintakód.</span><span class="sxs-lookup"><span data-stu-id="b6265-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="b6265-126">A mintakódban csak a vállalat entitássorainak kategorizálására szolgáló egyik mód jelenik meg.</span><span class="sxs-lookup"><span data-stu-id="b6265-126">The sample code shows only one of many ways you might categorize entity rows by company.</span></span> <span data-ttu-id="b6265-127">Ha megváltoztatja a logikát a **TENNIVALÓ** szakaszokban, egyéni kategorizálást hozhat létre.</span><span class="sxs-lookup"><span data-stu-id="b6265-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="b6265-128">Mi várható?</span><span class="sxs-lookup"><span data-stu-id="b6265-128">What should I expect?</span></span>
<span data-ttu-id="b6265-129">Alapértelmezés szerint a mintapályázattal az üzleti egységek és a vállalat közötti kódleképezések tára adható meg.</span><span class="sxs-lookup"><span data-stu-id="b6265-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="b6265-130">Az **OwningBusinessUnit** mező használatával rendszerindított entitás automatikusan a megadott vállalatot használja.</span><span class="sxs-lookup"><span data-stu-id="b6265-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="b6265-131">Az **OwningBusinessUnit** mező nélküli entitások (pl. egy termék) üres üzletiegység-értékkel való leképezés alapján állítja be a vállalatot.</span><span class="sxs-lookup"><span data-stu-id="b6265-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="b6265-132">A konzolpályázat egy paramétert vár: a **–szimuláció** vagy az **–alkalmaz** paramétert.</span><span class="sxs-lookup"><span data-stu-id="b6265-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="b6265-133">Ha a **–szimuláció** parancssori paramétert használja, akkor semmilyen adat nem frissül.</span><span class="sxs-lookup"><span data-stu-id="b6265-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="b6265-134">Csak **simulation_<entityname>.csv**-fájlok jönnek létre, ugyanabban a könyvtárban, mint amelyikben az eszköz van; minden olyan entitáshoz egy, amelyik frissülne.</span><span class="sxs-lookup"><span data-stu-id="b6265-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="b6265-135">Ezeket a fájlokat munka közben iteratívan ellenőrizheti, hogy a kód csakugyan a várt módon frissítse a vállalat értékeit.</span><span class="sxs-lookup"><span data-stu-id="b6265-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="b6265-136">Ha befejezte a szimulált frissítéseket, használja az **–alkalmaz** paramétert.</span><span class="sxs-lookup"><span data-stu-id="b6265-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="b6265-137">Ez frissíti az összes olyan sort, amelyen jelenleg hibás a vállalati érték, egyszerre 1000 soros kötegekben (alapértelmezés szerint).</span><span class="sxs-lookup"><span data-stu-id="b6265-137">This updates all rows that currently have an incorrect company value, in batches of 1000 rows at a time (by default).</span></span> <span data-ttu-id="b6265-138">A kód a megadott módon idempotens, vagyis újra futtatható, és csak a helytelenül hozzárendelt vállalatok lesznek frissítve.</span><span class="sxs-lookup"><span data-stu-id="b6265-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="b6265-139">Az **–alkalmaz** paraméter futtatásakor a kód az elvégzett változtatások CSV-fájljait adja eredményként; a fájlok neve **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="b6265-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
