...
-   Az oszlopdefiníciók fejlécek területe tartalmazza a fejlécszöveget és azt a formázást, amely a jelentésben szerepel. Fejléc alkalmazhat az adatok egy oszlopban, több oszlopot is kiterjedhet, vagy feltételes alapon mezőkre alkalmazható. Az oszlop definícióját tartalmazhatja annyi oszlopot fejlécsorok, szüksége van. **Megjegyzés:** Az oszlopfejlécek a jelentésben szereplő minden adatoszlopra vonatkoznak. A jelentésfejlécek a teljes jelentés vonatkoznak. A jelentésfejléceket a jelentésdefiníció **Fejlécek és láblécek** lapján definiálhatja.
...
Oszlopkorlátozások segítségével megadhatja, hogy egy oszlopdefiníció hogyan használja az adatokat vagy azámítson ki információkat. A jelentésoszlopot is korlátozhatja egy meghatározott egységre vagy megadott dátumokra vonatkozóan. **Megjegyzés:** A **Oszlopkorlátozás** kód felülírja az összes ellentmondó beállítást a sordefinícióban.
...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Amennyiben mindkét aláhúzás parancs tiltja le az oszlop az aláhúzásjel (**---**) vagy dupla aláhúzás parancs (**===**) szerepel a sordefiníció. Például előfordulhat, hogy nem szeretné aláhúzni azokat az összegeket, amelyeket egy százalékszámítás adott.                                                                        |
...
| UPT                     | Az összegek korlátozása az oszlopban úgy, hogy csak a fel nem adott tranzakciók jelenjenek meg, ha vannak ilyen tranzakciók elérhetőek. **Megjegyzés:** Nem minden adatszolgáltató támogatja a fel nem adott tranzakciók használatát. További tudnivalókért lásd a Microsoft Dynamics ERP [adatintegrációs útmutatóját](http://go.microsoft.com/fwlink/?LinkID=162565). |
...
| Naptári évben (@CalYear)                | Az oszlop naptéri évét számszerű formában nyomtatja ki.                                                                                                                                                                                                                                          |
| Kezdő dátuma (@StartDate)                 | Az oszlop kezdő dátumát nyomtatja ki.                                                                                                                                                                                                                                                             |
| Befejező dátuma (@EndDate)                     | Az oszlop záró dátumát nyomtatja ki.                                                                                                                                                                                                                                                               |
| A fában egység neve (@UnitName)         | Ha egy oszlopot a jelentésfa egy meghatározott egységére korlátozza, akkor az egység nevét nyomtatja ki az oszlopfejlécben.                                                                                                                                                                                     |
...
| Könyv kód (@BookCode)                   | Az oszlopban meghatározott könyvelési kódot nyomtatja ki.                                                                                                                                                                                                                                             |
...
-   Ha olyan oszlopfejléceket ad meg, amelyek átfedésben vannak, akkor a jelentés kinyomtatásakor átfedésben jelennek meg. A jelentés elkészül, de a következő figyelmeztetés jelenik meg a **a jelentés-várólista állapotának** mező: "oszlopfejlécek segítségével az Alap más oszlopfejlécek keresztezés és egymást átfedő szöveg vezethet." Például a fejléc meghatározása a B oszlop B alap + 1, és a D oszlop fejlécének meghatározása alap + 1 F. Ebben az esetben a fejlécek egymás tetejére nyomtatja és nem olvasható. Ha ALAP elemet használ az **Ettől terjed/Eddig terjed** definícióban, akkor mindig nézze meg a generált jelentést, hogy nem fedik-e át egymást a fejlécek.
...
-   Feltételes nyomtatási oszlopok (**P&lt;B**, **P&gt;= B**), feltételes kiterjedő fejlécek úgy viselkednek, mint bármely normál fej oszlopdefiníció. Ha például a feltétel hamis, minden olyan további oszlop, amely megegyezik a terjesztési feltétellel, elindítja a fejléc nyomtatását.
...
Phyllis duplán rákattint egy oszlopfejléc cellá, hogy megnyissa az **Oszlopfejléc** párbeszédablakot, és itt megadja az alábbi adatokat.
...
Az adatok bevitele után Phyllis az **OK** gombra kattint. Ezután duplán kattint a C oszlop fejléccellájára, hogy megnyissa az **Oszlopfejléc** párbeszédablakot, és itt megadja az alábbi adatokat.
...
3.  Válasszon egy oszloptípust a listából. A következő táblázat a különböző oszlopok típusait írja le.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Ha a könyvelési rendszere támogatja az attribútumok használatát, akkor számla- vagy tranzakcióattribútumot jelenít meg az oszlopban. Egy attribútum, amelynek vonatkoznia kell egyetlen teljes számlára, kivonatolja az alapul szolgáló számla- vagy tranzakcióadatokat a pénzügyi adatokból. Számla szintű jellemzőket a fiók adatainak megjelenítésére és tranzakció szintű jellemzőket történt a tranzakció feladásának időpontjában-adatok megjelenítése. Ha <strong>ATTR</strong> oszloptípust választja, adja meg a kategória attribútum a <strong>könyv kód/attribútum kategória</strong> az oszlop definícióját részletek sorában.</td>
    </tr>
    </tbody>
    </table>
...
| ALAP +\# | Használja, amely az év \#év a bázisév után. Az alapévet követő harmadik év használatához például írja be az **ALAP+3** kódot. |
| ALAP-\# | Használja, amely az év \#év a bázisév előtt. Az előző év használatához az **ALAP-1** kódot kell használnia.                 |
...
| ALAP +\#         | Használja az időszak, amely \#a bázisidőszak utáni időszakok. Az alapidőszakot követő harmadik időszak használatához például írja be az **ALAP+3** kódot.                                                                                               |
| ALAP-\#         | Használja az időszak, amely \#a bázisidőszak előtti időszakok. Az előző időszak használatához az **ALAP-1** kódot kell használnia.                                                                                                                 |
...
Bármely időszakmeghatározásban túlléphet a pénzügyi év határokon, és vegyítheti az éveket az időszakoktartományokban. Például a pontot adja meg **BASE-5** (jelölésére az elmúlt hat), majd futtassa a jelentést, amely a bázisidőszak 2. Ebben az esetben a jelentés az adott pénzügyi év első két időszakok és az előző pénzügyi év utolsó négy időszakok adatokat jelenít meg.
...
3.  A navigációs ablak feletti képletsorban vagy az **Időszak** cellában töltse ki a képletet. Cserélje ki a kettős kereszt (\#) a megfelelő értékkel.
...
Dimenziószűrő használatával korlátozhatók az **FD** oszlopok a dimenzióértékek megadásához. A szűrő tartalmazhat egyetlen dimenziót, dimenziótartományt vagy a dimenziók csoportját. A szűrő is tartalmazhat dimenzióérték-készleteket. Dimenzió értékek eltérőek lehetnek, mert a... \financial-dimensions\dimension-based rendszer nem kell megegyeznie a pontos hosszát. A szűrő alkalmazására kerül sor, függetlenül attól, hogy a jelentés tartalmaz-e jelentési fát. Használhat helyettesítő karaktert (\* vagy?), egy helyen. Több fiókot adja meg, ha például a számlák között vessző helyezze: + számla =\[1200\], + számla =\[1100\], részleg =\[01?\] Ahhoz, hogy egy adott számla minden részlegét megkapja, kizárhatja a Részleg dimenziót a dimenziószűrőből. Például a következő dimenziószűrők ugyanúgy lesznek kezelve:

-   + Számla =\[1100\], szervezeti egység
-   + Számla =\[1100\]

Használhatja továbbá az alfanumerikus karakterek bármely kombinációját a pontos találatokra, és részleges dimenziókat is meghatározhat. Például **helye = \[10\*\]** hely dimenzió értékek kezdődik a 10.
...
**Megjegyzés:** Az itt leírt lehetőségek a használt ERP rendszertől függően eltérhetnek. További tudnivalókért tanulmányozza a [a Microsoft ERP rendszerre vonatkozó dokumentáció](https://www.microsoft.com/en-us/download/details.aspx?id=5916).
...
Az alábbi táblázat bemutatja a jelentés különböző eredményeit, amelyek a **Pénznem megjelenítése** és a **Pénznemszűrő** cellák Phyllis által kiválasztott beállításainak különböző kombinációi esetén létrejöhetnek. A működési pénznem USD.
...
| Működési pénznem a vállalati adatokból | Üres                | **$2,310\*\*** – az eredmény minden adat látható a funkcionális pénznemében van megadva a vállalati információkban. **Megjegyzés:** Ez az összeg a tranzakciók összesítése, működési pénznemben. |
...
Oszlop típusú **CALC** oszlopban definition támogatja az összetett számítások a **képlet** cella, és tartalmazhatja a **+**, **-**, **\***, és **/**piaci szereplők, és **IF/THEN/ELSE** kimutatások. A számítás oszlopban is hivatkozhat más oszlopot, még további oszlopokat. Emellett a számítási oszlopok tartalmazhatják a pénzügyi évet és időszakot is, így támogatják az oszlopfejlécet. A számítási képlet legfeljebb 1,024 karakterből állhat. A számítás eredményének százalékban való kifejezéséhez használjon egy speciális formátumfelülírást. **Megjegyzés:** A számítási képletek eredménye nem tartalmaz nem nyomtatandó oszloptartományokból származó értékeket. Például az **A:D** kód a **0** (nulla) összeget nyomtatja ki, míg az **A+B+C** képlet nem nyomtatandó értékekre kiszámítja az értéket.
...
| -        | A-C                 | Kivonja az A oszlop összegét a C oszlop összegéből. **Megjegyzés:** A mínusz jelet (-) arra is használhatja, hogy megfordítsa egy oszlop összegeinek előjelét. Például az **-A+B** képlettel az A oszlop negatív fordítottjának összegeit adhatja hozzá a B oszlophoz. |
